...
The price included an Ichijo-partner discount of 45% for the curtains (but not for the installation), so their list price would have been almost twice this much. We actually didn't get all the lace curtains initially, as we had some left-over curtains from previous apartments, and only after moving in did we realize that we wanted more. So we ordered the bedroom lace curtains later, but Jias let us still use the Ichijo discount.
